...
export const useOpenData = (code: string, resource_id: string) => {
  return useQuery({
    queryKey: ["gushitong-opendata"],
    queryFn: () => {
      return http<Gushitong<any>>(
        "https://meme.us.kg:8888/proxy/gushitong.baidu.com/opendata",
        { params: { code, resource_id, query: code } },
      ).then((d) => {
        if (d.ResultCode !== "0" && d.ResultCode !== 0) {
          throw new Error(`ResultCode:${d.ResultCode}, is not success`);
        }
        const result = d.Result[0]?.DisplayData.resultData.tplData.result;
        const data: OpenData = {
          name: result.name,
          code: result.code,
