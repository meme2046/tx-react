...
import type { ArweaveData, StrategyTx } from "@/types";

export const newEmptyArweaveTx = (nodeId: string): ArweaveData => {
  return {
...
const newArweaveTx = (index: number): StrategyTx => {
  if (index % 2 == 0) {
    return {
      cex: "binance",
...
export function makeData(...lens: number[]) {
  const makeDataLevel = (depth = 0): StrategyTx[] => {
    const len = lens[depth]!;
    return range(len).map((d): StrategyTx => {
